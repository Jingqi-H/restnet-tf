"""
这个main2.py文件，用的是迁移学习方法二中的，加载预训练模型，只训练最后全连接层
"""
...
def cross_valid(
    ...
    for i in range(k_fold):
        ...
        # "net.fc"对应model.py的107行,".in_features"是指输入特征矩阵的深度
        inchannel = net.fc.in_features
        net.fc = nn.Linear(inchannel, num_classes)  # 这里重新赋值我们的全连接层
        net.cuda()
        ...
        optimizers = optim.Adam({'params': net.fc.parameters()}, lr=learning_rate, weight_decay=weight_decay)
        ...
        save_dir = os.path.join(os.path.join('./result', date), 'main2')
        mkfile(save_dir)
...
if __name__ == '__main__':
    ...
    print('I am Tf2 with  on data with strip. / lr={} / wd={}.'.format(opt.lr, opt.weight_decay))
    ...
    save_dir = os.path.join(os.path.join('./result', opt.date), 'main2')
